...
// TUMIA onValue BADALA YA get - INAKUA REALTIME
function loadAllData(){
  document.getElementById('promptGrid').innerHTML = `<p style='text-align:center; grid-column:1/-1; color:#aaa'>Inapakia prompts...</p>`;
  const promptsRef = ref(db, 'prompts');
  
  onValue(promptsRef, (snapshot) => {
    allPrompts = [];
    if(snapshot.exists()){
      snapshot.forEach(c => allPrompts.push({id: c.key, likes: 25, ...c.val()}));
    }
    console.log("Prompts zilizopatikana:", allPrompts.length);
    loadFiltersAndCounts();loadPrompts();loadHighlights();loadAdminList();
  }, (error) => {
    console.error("ERROR KUSOMA FIREBASE:", error);
    document.getElementById('promptGrid').innerHTML = `<p style='color:red'>ERROR: ${error.message}. Angalia Rules za Firebase</p>`;
  });
}
...
function loadHighlights(){
  ...
  const trending = [...allPrompts].sort((a,b) => (b.likes || 25) - (a.likes || 25)).slice(0,4);
...
function loadPrompts(filter = 'all', searchTerm = ''){
  ...
  if(filtered.length > 0) filtered.forEach(p => {
    ...
    <div class="prompt-card" id="${p.id}" data-cat="${p.cat}">
      <img src="${p.img}" class="prompt-img" onerror="this.src='https://placehold.co/400x400/000/00F5FF?text=IMAGE+ERROR'">
      <div class="prompt-body">
        ...
        <div class="action-bar">
          <div class="action-left">
            <button class="action-btn like-btn ${isLiked ? 'liked' : ''}" onclick="likePost('${p.id}')"><i class="fa-${isLiked ? 'solid' : 'regular'} fa-heart"></i> <span>${(p.likes || 25) + (isLiked ? 1 : 0)}</span></button>
  ...
  else grid.innerHTML = `<p style='text-align:center; grid-column:1/-1; color:#aaa'>${currentLang === 'sw' ? 'Hakuna prompt. Data: '+allPrompts.length : 'No prompts. Data: '+allPrompts.length}</p>`;
}
...
window.savePrompt = async () => {const btn = document.querySelector('.btn-save');const catSelect = document.getElementById('cat').value;const catInput = document.getElementById('newCatTemp').value.toLowerCase().trim();const finalCat = catInput !== "" ? catInput : catSelect;const data = { cat: finalCat, text: document.getElementById('promptText').value, img: document.getElementById('imgLink').value, likes: 25 };if(data.text === "" || data.img === ""){ document.getElementById('saveMsg').innerText = currentLang === 'sw' ? "Jaza prompt na link ya picha" : "Fill prompt and image link"; return; }btn.disabled = true;if(editingId){ btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> ' + (currentLang === 'sw' ? 'Inasasisha...' : 'Updating...'); await update(ref(db, 'prompts/' + editingId), data); document.getElementById('saveMsg').innerText = "✓ " + (currentLang === 'sw' ? 'Imesasishwa!' : 'Updated!'); editingId = null; } else { btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> ' + (currentLang === 'sw' ? 'Inahifadhi...' : 'Saving...'); const id = Date.now(); await set(ref(db, 'prompts/' + id), data); document.getElementById('saveMsg').innerText = "✓ " + (currentLang === 'sw' ? 'Imehifadhiwa!' : 'Saved!'); }btn.disabled = false;btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> ' + (currentLang === 'sw' ? 'HAZIFA PROMPT' : 'SAVE PROMPT'); resetForm(); }
...
import { update, remove, set } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-database.js";
...
window.delPrompt = async (id) => { if(confirm(currentLang === 'sw' ? "Una uhakika?" : "Are you sure?")){ await remove(ref(db, 'prompts/' + id)); } }
