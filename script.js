...
const texts = {
  sw: {lib:"LIBRARY", wa:"WhatsApp", fb:"Facebook", rights:"© 2026 Nova Soft. Haki Zote Zimehifadhiwa. Inaendeshwa na AI", lang:"🌍 EN", readMore:"SOMA ZAIDI", like:"❤️", copy:"📋 NAKILI", open:"OPEN IN CHATGPT", noPrompts:"Hakuna prompts bado"},
  en: {lib:"LIBRARY", wa:"WhatsApp", fb:"Facebook", rights:"© 2026 Nova Soft. All Rights Reserved. Powered by AI", lang:"🌍 SW", readMore:"READ MORE", like:"❤️", copy:"📋 COPY", open:"OPEN IN CHATGPT", noPrompts:"No prompts yet"}
}
  ...
  db.ref('prompts').orderByChild('createdAt').on('value', (snapshot) => {
    ...
    if(data){
      Object.keys(data).reverse().forEach(key => {
        ...
        container.innerHTML += `<div class="card">${item.image? `<img src="${item.image}" class="card-img">` : ""}<span class="badge">${item.category}</span><p class="prompt-text" id="text-${key}">${shortText}</p>${item.prompt.length > 120? `<a onclick="toggleText('${key}', \`${safePrompt}\`)">${texts[lang].readMore}</a>` : ""}<div class="actions"><button onclick="likePrompt('${key}', ${item.likes})">${texts[lang].like} ${item.likes}</button><button onclick="copyPrompt(\`${safePrompt}\`)">${texts[lang].copy}</button><button onclick="openChatGPT(\`${safePrompt}\`)">${texts[lang].open}</button></div></div>`;
      });
    } else {
...
function likePrompt(id, current){ db.ref('prompts/'+id+'/likes').set(current + 1); }
function copyPrompt(text){ navigator.clipboard.writeText(text).then(()=>{ alert(lang==='sw'? "Imenakiliwa!" : "Copied!"); }); }
